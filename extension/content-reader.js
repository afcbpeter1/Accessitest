(function () {
  ...
  function speak(text) {
    ...
    // ElevenLabs API (and our server route) hard-limits TTS text length.
    // Focus announcements can be long (e.g., links or rich components), so we
    // truncate to ensure we always get audio back.
    var MAX_TTS_CHARS = 240;
    if (text && text.length > MAX_TTS_CHARS) {
      var cut = text.slice(0, MAX_TTS_CHARS);
