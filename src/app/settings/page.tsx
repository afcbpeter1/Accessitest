...
export default function SettingsPage() {
  ...
  // User role and organization state
  ...
  // Load organization after user is loaded
  useEffect(() => {
    if (user) {
  ...
  // Load integrations only if user is admin/owner
  useEffect(() => {
    if ((userRole === 'owner' || userRole === 'admin') && activeTab === 'integrations') {
      loadJiraIntegration()
      if (integrationSubTab === 'azure') {
        loadAzureDevOpsIntegration()
      }
    }
  }, [activeTab, integrationSubTab, userRole])
  ...
  const loadJiraIntegration = async () => {
    try {
      ...
      // Always load personal integration in settings
  ...
  const handleJiraSave = async () => {
    ...
    try {
      ...
      // Settings page only saves personal integrations (no teamId)
